...
final class Srcset
{
    ...
    public function __construct($data = null, array $options = [])
    {
        if (is_a($data, "Kirby\Cms\KirbyTag")) {
            ...
            is_a($data, "Kirby\Cms\File") ||
            is_a($data, "Kirby\Cms\FileVersion")
        ) {
            $this->parent = $data->parent();
        ...
        if ($this->parent && !A::get($this->options, "file")) {
            $this->options["file"] = $this->parent->file(
                (string) A::get($this->options, "value")
            );
        }
    ...
    public function imageKirbytagFromData(array $data = []): string
    {
        ...
        if ($lazy) {
            $data["imgclass"] = trim(
                A::get($data, "imgclass", "") . " " . $lazy
            );
        }
        if (A::get($data, "ratio") && !A::get($data, "caption")) {
            $data["class"] = trim(
                A::get($data, "class", "") . " " . A::get($data, "ratio")
            );
        }
